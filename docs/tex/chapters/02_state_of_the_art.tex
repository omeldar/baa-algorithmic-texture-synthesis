...
To correctly map textures onto geometry, each point on the surface must correspond to coordinates in the texture image. These coordinates are typically called \textit{UV coordinates}. The letters $U$ and $V$ refer to the axes of the two-dimensional texture space and represent the horizontal and vertical directions of the texture image \cite{wiki_texture_mapping}. In practice, the surface of a 3D model is parameterized into this two-dimensional coordinate system so that texture images can be applied to it.
...
\section{Texture Similarity Metrics and Evaluation}

When evaluating texture synthesis methods, it is necessary to measure how similar a generated texture is to a target texture. Since textures often contain stochastic patterns and local variations, comparing them directly on a pixel-by-pixel basis is usually not sufficient. Two textures may look visually similar even if their individual pixel values differ significantly. Therefore, evaluation methods often rely on statistical or perceptual similarity metrics \cite{ebert2003texturing}.

One common approach is to compare statistical properties of textures. Textures can be characterized by features such as color distributions, spatial correlations, or frequency patterns. For example, color histograms can be used to compare the distribution of colors between two textures, while frequency-based analysis can measure how patterns repeat at different spatial scales. By comparing these statistical features, it is possible to estimate how closely a generated texture matches the structural characteristics of a target texture \cite{ebert2003texturing}.

Another widely used metric is the \textit{Structural Similarity Index (SSIM)}. SSIM measures the similarity between two images by comparing local patterns of pixel intensities that have been normalized for luminance and contrast. Unlike simple metrics such as mean squared error, SSIM attempts to model aspects of human visual perception and is therefore often better suited for evaluating visual similarity \cite{wang2004ssim}. Because of this property, SSIM is commonly used in image processing and computer graphics to assess image quality.

More advanced approaches use feature-based representations of textures. For example, the parametric texture model proposed by Portilla and Simoncelli compares images based on statistical features derived from wavelet transforms \cite{portilla2000parametric}. By matching these statistical descriptors, textures can be compared based on their structural properties rather than their exact pixel values.

Despite the availability of quantitative metrics, evaluating texture synthesis methods often also requires qualitative analysis. Human observers are very sensitive to visual artifacts such as repetition patterns, discontinuities, or unnatural structures. As a result, visual inspection is frequently used together with quantitative metrics to assess the quality of synthesized textures \cite{akenine2018realtime}.

\clearpage
In the context of this thesis, texture similarity metrics are used to evaluate how well algorithmically generated textures approximate a given target texture. Both quantitative measures and qualitative visual comparisons may be considered in order to analyze the strengths and limitations of the investigated approaches.
