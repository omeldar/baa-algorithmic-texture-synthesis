...
To correctly map textures onto geometry, each point on the surface must correspond to coordinates in the texture image. These coordinates are typically called \gls{uv}. The letters $U$ and $V$ refer to the axes of the two-dimensional texture space and represent the horizontal and vertical directions of the texture image \cite{wiki_texture_mapping}. In practice, the surface of a \gls{3d} model is parameterized into this two-dimensional coordinate system so that texture images can be applied to it.
...
By combining multiple noise functions with different frequencies and amplitudes, more complex patterns can be created. A common technique is \gls{fbm}, which generates fractal-like structures by summing several layers of noise at different scales \cite{ebert2003texturing}. These layered noise functions allow procedural models to simulate complex natural phenomena while still being controlled by a small number of parameters.

\begin{figure}[H]
    \centering
    \img[width=1\textwidth]{figures/02/perlin-octaves-comparison.png}
    {Illustration of \gls{fbm} using Perlin noise. Increasing numbers of octaves are combined from left to right, adding higher-frequency detail at each step and producing progressively more complex, multi-scale structures.}
...
Optimization-based texture approximation describes methods that adjust the parameters of a generative texture model so that its output becomes closer to a selected target texture. Instead of copying pixels from the input image directly, the target image is used as a reference and the texture model is treated as a search problem. The goal is to find parameter values that minimize the difference between the generated texture and the reference according to a defined similarity measure \cite{ebert2003texturing}.

This is especially relevant for procedural textures, because their appearance is controlled through parameters such as scale, color, frequency, contrast, or structural detail. Optimization can therefore be used to search this parameter space automatically instead of relying only on manual tuning \cite{akenine2018realtime}. A central challenge is the choice of the similarity metric. Simple metrics may compare color distributions or pixel statistics, while more advanced approaches use perceptual or feature-based measures. Statistical texture models, such as the work by Portilla and Simoncelli, show that matching image statistics can reproduce characteristic texture appearances \cite{portilla2000parametric}. Neural texture synthesis follows a similar idea with learned feature representations \cite{gatys2015texture}.

However, optimization-based approaches are usually computationally expensive, because many candidate textures must be generated and evaluated. In addition, a good numerical score does not always guarantee a visually convincing result. For this thesis, optimization is therefore treated as an offline parameter approximation method rather than as a real-time texture generation technique.


\section{Applications of Algorithmic Texture Synthesis}

Algorithmic texture synthesis is used in computer graphics to create scalable and varied surface detail without storing every texture as a static image. This is especially useful in real-time applications and procedural content generation, where large environments require many different materials while memory usage and manual asset creation should remain limited \cite{akenine2018realtime, ebert2003texturing, shaker2016pcg}.

Typical use cases include terrain, vegetation, building materials, natural effects, and visual effects production. Because algorithmic textures are parameterized, their appearance can be adapted more easily than fixed image textures. In the context of this thesis, the most relevant application is procedural real-time environments, where generated textures can support variation, scalability, and controllability.

\section{Existing Systems and Tools for Texture Synthesis}

Various systems and tools for procedural texture synthesis already exist in industry and research. Dedicated material tools such as Adobe Substance \gls{3d} Designer \cite{substance_designer} and Material Maker \cite{material_maker} use node-based workflows to create textures and materials through procedural operations. Larger graphics environments such as Blender \cite{blender}, Houdini \cite{houdini}, and Unreal Engine \cite{unreal_engine} also integrate procedural texturing into rendering and content creation pipelines.

\clearpage
Research continues to investigate topics such as semantic procedural generation, optimization-based texture approximation, and \gls{ai}-assisted material generation \cite{wang2017semantic}. These approaches aim to automate procedural workflows and improve the approximation of target textures.

Although existing systems provide powerful procedural capabilities, they are mainly designed for production workflows rather than controlled scientific comparison. Many tools hide implementation details and include internal optimizations, which makes reproducible runtime measurements and direct algorithm comparison more difficult. Therefore, this thesis uses a dedicated research prototype for controlled evaluation of procedural and rule-based texture synthesis methods in a real-time environment.

\section{Research Gap}

The reviewed approaches show that procedural, rule-based, and optimization-based texture synthesis each offer different advantages for algorithmic texture generation. However, a practical comparison of these approaches for approximating a specific target texture remains necessary. In particular, it is not sufficient to only describe the methods theoretically, because their usefulness depends on how well they can be implemented, controlled through parameters, and evaluated against a reference texture.

The research gap addressed in this thesis is therefore the practical investigation of how different algorithmic texture synthesis approaches can approximate a given target texture. The focus lies on procedural methods, rule-based methods, and optimization-based parameter estimation. These approaches are implemented and compared with respect to visual similarity, flexibility, reproducibility, parameter controllability, and suitability for typical computer graphics applications.

Based on this gap, the next chapter defines the methodological approach used in this thesis. It explains how the use case is selected, how the texture generation methods are implemented, and how the generated results are evaluated against the target texture.