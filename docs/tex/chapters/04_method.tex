...
Within each phase, a Plan-Do-Check-Act (PDCA) inspired workflow was used. Planned functionality and research objectives were implemented incrementally and regularly reviewed based on technical feasibility, experimental usefulness, and runtime behavior. This iterative approach made it possible to adapt development decisions when new challenges or requirements emerged during implementation.
...
AI-assisted tools were used throughout the project as supportive development and writing aids. Their usage primarily included assistance with research, conceptual understanding and refinement of the English language within the written documentation.

Since English is not the native language of the author, AI tools were particularly helpful for improving readability, grammar, and phrasing of technical explanations. However, implementation, implementation decisions, system architecture, experimental design, and scientific conclusions were developed and evaluated independently by the author.
...
Several evaluation criteria are used to investigate the implemented texture synthesis approaches.

Visual quality is evaluated through qualitative comparison and user feedback collected using evaluation forms. Participants compare generated textures and assess their visual similarity, variation, and overall appearance.

Performance evaluation focuses on runtime-related metrics such as frames per second (FPS) and texture generation time. These measurements allow the computational costs of different synthesis methods to be compared under identical conditions.

Another important evaluation criterion is parameter controllability. This investigates how effectively procedural parameters can be adjusted in order to approximate target textures and influence generated visual structures.

In addition, reproducibility and consistency are considered important aspects of the evaluation due to the research-oriented focus of the prototype.

\subsection{Reproducibility and Controlled Experiments}

To ensure reproducibility, the prototype uses deterministic generation techniques based on configurable seeds and parameter sets. This allows experiments to be repeated consistently under identical conditions.

Controlled experiments are performed by modifying individual parameters or synthesis methods while keeping other environmental conditions constant. This helps isolate the influence of specific algorithms and parameter configurations on generated textures and runtime behavior.

The use of a unified procedural environment further improves comparability between different synthesis approaches since all methods are evaluated within the same rendering and scene generation framework.
